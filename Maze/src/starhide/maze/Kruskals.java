		...
		for (int count = 0; count < 9; count++) {
			for (int index = 0; index < edges.size(); index++) {
				if (vertices[edges.get(index).getFirst()] == vertices[edges.get(index).getSecond()]) {
					...
					grid[edges.get(index).getSecond() % width * 2 + 1][edges.get(index).getSecond() / width * 2
							+ 1] = 0;
					grid[edges.get(index).getFirst() % width * 2 + 1][edges.get(index).getFirst() / width * 2 + 1] = 0;
					grid[((edges.get(index).getSecond() % width) - (edges.get(index).getFirst() % width))
							+ (edges.get(index).getFirst() % width) * 2
							+ 1][((edges.get(index).getSecond() / width) - (edges.get(index).getFirst() / width))
									+ (edges.get(index).getFirst() / width) * 2 + 1] = 0;
